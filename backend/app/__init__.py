...
def create_app(config_name=None):
    ...
    def health_check():
        ...
        try:
            import requests
            ml_url = app.config.get('ML_SERVICE_URL', 'http://localhost:5001')
            res = requests.get(f"{ml_url}/health", timeout=3)
            ml_ok = (res.status_code == 200)
        except Exception:
            ml_ok = False

        status = "healthy" if (db_ok and ml_ok) else "degraded"
        http_code = 200 if (db_ok and ml_ok) else 503

        return {
            "status": status,
            "service": "sirkula-backend",
            "database": "connected" if db_ok else "disconnected",
            "ml_service": "connected" if ml_ok else "disconnected",
        }, http_code

    # Serve uploaded files (KTP, rewards, etc)

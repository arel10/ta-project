...
class WasteDeposit(db.Model):
    ...
    waste_type = db.Column(db.String(32), nullable=False)  # plastik, kertas, logam, kaca, organik, elektronik
    activity_type = db.Column(db.String(120), nullable=True)
    source_waste_label = db.Column(db.String(120), nullable=True)
    source_price_per_kg = db.Column(db.BigInteger, nullable=True)
    source_total_savings = db.Column(db.BigInteger, nullable=True)
    status = db.Column(db.Enum('pending', 'validated', 'rejected', name='deposit_status_enum'), nullable=False, default='pending')

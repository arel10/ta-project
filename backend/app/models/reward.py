...
class RewardRedemption(db.Model):
    ...
    status = db.Column(db.Enum('pending', 'approved', 'rejected', name='redemption_status_enum'), nullable=False, default='pending')
    rejection_reason = db.Column(db.String(255), nullable=True)
    redemption_code = db.Column(db.String(24), unique=True, nullable=False)
    created_at = db.Column(db.DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))

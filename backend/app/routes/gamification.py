...
def get_missions():
    ...
    # Get user's current churn status
    risk_profile = ParticipationRisk.query.filter_by(user_id=user.id).first()
    user_churn_status = 'churn' if (risk_profile and risk_profile.will_churn) else 'not_churn'

    active_missions = Mission.get_active_query().all()
    result = []

    for mission in active_missions:
        # Filter by target_label: if set, only show to matching churn-status users
        mission_target = mission.target_label  # None / 'churn' / 'not_churn'
        if mission_target and mission_target != user_churn_status:
            continue  # skip — not targeted at this user's churn status

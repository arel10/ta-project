...
def get_members():
    ...
    churn_filter = request.args.get('churn_filter', '').strip().lower()
    status_filter = request.args.get('status_filter', '').strip().lower() or request.args.get('status', '').strip().lower()
    ...
    if status_filter in ('pending', 'approved', 'rejected'):
        ...
        # Default: only show active/approved members in main members list
    ...
    if search:
        query = query.filter(
            db.or_(
                ...
                User.nik.ilike(f'%{search}%'),
                User.phone.ilike(f'%{search}%'),
            )
        )

    if churn_filter in ('churn', 'not_churn'):
        query = query.join(
            ParticipationRisk,
            ParticipationRisk.user_id == User.id,
        ).filter(ParticipationRisk.will_churn == (churn_filter == 'churn'))
    ...
    if member_ids:
        ...
        risk_map = {r.user_id: r for r in risks}

    results = []
    for m in members.items:
        member_data = m.to_dict()
        risk_record = risk_map.get(m.id)
        member_data['will_churn'] = risk_record.will_churn if risk_record else None
        member_data['churn_probability'] = risk_record.churn_probability if risk_record else None
...
def get_member_detail(member_id):
    ...
    return jsonify({
        ...
        "churn_profile": risk.to_dict() if risk else None,
...
@admin_bp.route('/members/<int:member_id>/approve', methods=['PUT'])
@jwt_required()
def approve_member_verification(member_id):
    """Admin approves a pending member account registration."""
    user = _get_current_user()
    admin_check = _require_admin(user)
    if admin_check:
        return admin_check
    ...
    member = User.query.get(member_id)
    if not member or member.role != 'member':
        return error_response("Anggota tidak ditemukan", "not_found", status=404)

    member.status = 'approved'
    db.session.commit()

    return jsonify({
        "message": "Akun anggota berhasil disetujui & diaktifkan",
        "member": member.to_dict(),
    }), 200


@admin_bp.route('/members/<int:member_id>/reject', methods=['PUT'])
@jwt_required()
def reject_member_verification(member_id):
    """Admin rejects a pending member account registration."""
    user = _get_current_user()
    admin_check = _require_admin(user)
    if admin_check:
        return admin_check
    ...
    return jsonify({
        "message": "Akun anggota berhasil ditolak",
        "member": member.to_dict(),
    }), 200

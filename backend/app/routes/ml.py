...
from app.services.ml_service import predict_single, predict_batch, get_churn_summary, get_churn_trend
...
def analyze_user(user_id):
    """Admin triggers churn prediction for a single user."""
    try:
        ...
        if result is None:
            return jsonify({
                "message": "Tidak dapat menganalisis user. Pastikan user memiliki minimal 2 setoran tervalidasi dan ML Service aktif."
            }), 400

        return jsonify({
            "message": "Analisis prediksi churn berhasil",
            "churn_profile": result,
        }), 200
...
def analyze_all_users():
    """Admin triggers batch churn prediction for all users."""
    try:
        admin = _get_current_user()
        admin_check = _require_admin(admin)
        if admin_check:
...
@ml_bp.route('/churn-summary', methods=['GET'])
@jwt_required()
def churn_summary():
    """Admin dashboard: churn distribution and predicted-churn user list."""
    admin = _get_current_user()
    admin_check = _require_admin(admin)
    if admin_check:
        return admin_check
...
@ml_bp.route('/churn-trend', methods=['GET'])
@jwt_required()
def churn_trend():
    """Admin dashboard: monthly churn prediction distribution."""
    admin = _get_current_user()
    admin_check = _require_admin(admin)
    if admin_check:
        return admin_check

    trend = get_churn_trend(months=6)

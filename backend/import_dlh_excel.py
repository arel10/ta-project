...
from app.routes.admin_common import _generate_unique_import_email, _normalize_gender
...
def _resolve_waste_type(raw_jenis: object, valid_codes: set[str], name_to_code: dict[str, str]) -> str | None:
    ...
    if "plastik" in low or "pet" in low or "botol" in low or "ember" in low or low.startswith("p"):
        return "plastik"
    if "kertas" in low or "kardus" in low or "hvs" in low or "koran" in low or low.startswith("k"):
        return "kertas"
    if "besi" in low or "kaleng" in low or "seng" in low or "logam" in low or "minyak" in low or low.startswith("b") or low.startswith("l"):
        return "logam"
    if "kaca" in low:
        return "kaca"
    if "organik" in low:
        return "organik"
    if "elektronik" in low or "hp" in low or "laptop" in low:
        return "elektronik"
...
def import_dlh_data(
    *,
    ...
    if customer_sheet_name:
        ...
        if customer_cols["account_number"] is not None:
            for row in customer_ws.iter_rows(min_row=2, values_only=True):
                account_number = _clean_account_number(_get_val(row, customer_cols["account_number"]))
                if not account_number:
                    continue
                customer_map[account_number] = {
                    "name": _safe_text(_get_val(row, customer_cols["name"])),
                    "gender": _safe_text(_get_val(row, customer_cols["gender"])),
                    "nik": _safe_text(_get_val(row, customer_cols["nik"])),
                    "address": _safe_text(_get_val(row, customer_cols["address"])),
                    "department": _safe_text(_get_val(row, customer_cols["department"])),
                }
    ...
    for row in tx_ws.iter_rows(min_row=2, values_only=True):
        ...
        account_number = _clean_account_number(_get_val(row, tx_cols["account_number"]))
        if not account_number:
            stats["rows_skipped"] += 1
            stats["missing_account"] += 1
            continue

        weight_kg = _safe_float(_get_val(row, tx_cols["weight"]))
        if weight_kg is None or weight_kg <= 0:
            stats["rows_skipped"] += 1
            stats["invalid_weight"] += 1
            continue

        raw_waste = _get_val(row, tx_cols["waste"])
        ...
        created_at_raw = _get_val(row, tx_cols["timestamp"])
        created_at = _as_utc(created_at_raw if isinstance(created_at_raw, datetime) else None)
        raw_act = _safe_text(_get_val(row, tx_cols["activity_type"]))
        activity_type = raw_act[:120] if raw_act else None
        source_waste_label = str(raw_waste)[:120] if raw_waste else None
        source_price_per_kg = _safe_int(_get_val(row, tx_cols["price"]))
        source_total_savings = _safe_int(_get_val(row, tx_cols["total"]))
        tx_name = _safe_text(_get_val(row, tx_cols["name"]))
        ...
        if not user:
            ...
            if not user:
                ...
                user_name = str(customer_info.get("name") or tx_name or account_number)[:120]
                generated_email = _generate_unique_import_email(account_number)
                gender_val = _normalize_gender(customer_info.get("gender"))
                nik_val = str(customer_info["nik"])[:32] if customer_info.get("nik") else None
                address_val = str(customer_info["address"])[:255] if customer_info.get("address") else None
                dept_val = str(customer_info["department"])[:120] if customer_info.get("department") else None
        ...
        user_updated = False
        if customer_info:
            c_name = str(customer_info["name"])[:120] if customer_info.get("name") else None
            c_gender = _normalize_gender(customer_info.get("gender"))
            c_nik = str(customer_info["nik"])[:32] if customer_info.get("nik") else None
            c_addr = str(customer_info["address"])[:255] if customer_info.get("address") else None
            c_dept = str(customer_info["department"])[:120] if customer_info.get("department") else None

            if c_name and user.name != c_name:
                user.name = c_name
                user_updated = True
            if c_gender and user.gender != c_gender:
                user.gender = c_gender
                user_updated = True
            if c_nik and user.nik != c_nik:
                user.nik = c_nik
                user_updated = True
            if c_addr and user.address != c_addr:
                user.address = c_addr
                user_updated = True
            if c_dept and user.department != c_dept:
                user.department = c_dept
                user_updated = True
        elif tx_name and user.name != tx_name[:120]:
            user.name = tx_name[:120]
            user_updated = True
    ...
    if touched_user_ids:
        ...
        user_ids_list = list(touched_user_ids)
        chunk_size = 500
